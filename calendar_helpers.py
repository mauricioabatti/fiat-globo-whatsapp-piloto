...
def build_gcal(sa_b64: str, cal_id: str):
    if not (sa_b64 and cal_id):
...
def _to_local_naive(dt_str: str, tzinfo) -> datetime:
    dt = datetime.fromisoformat(dt_str.replace("Z", "+00:00"))
    return dt.astimezone(tzinfo).replace(tzinfo=None)

def freebusy(svc, d: date, tz: str, tzinfo, cal_id: str):
    start, end = business_hours_for(d, tzinfo)
    body = {"timeMin": start.isoformat(), "timeMax": end.isoformat(), "timeZone": tz, "items": [{"id": cal_id}]}
...
def is_slot_available(svc, start_dt: datetime, tzinfo, cal_id: str, tz: str) -> bool:
    ...
    if not (bh_start <= start_dt < bh_end): return False

    for s, e in freebusy(svc, start_dt.date(), tz, tzinfo, cal_id):
        if (s < end_dt) and (start_dt < e):
            return False
    return True

def create_event(svc, *, tzinfo, tz, calendar_id: str, tipo, nome, carro, cidade, telefone, start_dt: datetime):
    start_dt = start_dt.replace(minute=0, second=0, microsecond=0)
    end_dt = start_dt + timedelta(hours=1)
